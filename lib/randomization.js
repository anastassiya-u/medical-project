...
export function selectFoilCases(interventionCases, seed) {
  const totalCases = interventionCases.length; // Should be 10
...
export function prepareCaseForUser(caseData, accuracyLevel, foilCaseIds) {
  ...
  return {
    ...caseData,
    isFoil: isFoilCase,
    // Foil cases: swap in the wrong AI recommendation; non-foil keeps authored aiRecommendation
    ...(isFoilCase && {
      aiRecommendation: caseData.foilDiagnosis,
      aiRecommendation_ru: caseData.foilDiagnosis_ru,
    }),
  };
}
