...
/**
 * Get the vitals from a case, filtering out placeholder "Not reported" values.
 * Returns an object containing only vitals that have real data.
 * Returns null if no real vitals exist at all.
 */
export function getRealVitals(vitals) {
  if (!vitals || typeof vitals !== 'object') return null;
  const PLACEHOLDER = /^not reported$/i;
  const real = Object.fromEntries(
    Object.entries(vitals).filter(([, v]) => v && !PLACEHOLDER.test(String(v).trim()))
  );
