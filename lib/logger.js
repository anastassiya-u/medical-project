...
class ExperimentLogger {
  ...
  async submitLikertAssessment(responses) {
    await supabase.from('likert_assessments').insert({
      user_id: this.userId,
      session_id: this.sessionId,
      ...responses,
    });
  ...
  async startSession(sessionType) {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        total_cases: sessionType === 'intervention' ? 15 : 5,
      })
      .select()
      .single();
...
/**
 * Helper: Calculate learning gain for a user
 */
export async function calculateLearningGain(userId) {
  // Pre-test accuracy
  const { data: preTest } = await supabase
    .from('case_interactions')
    .select('user_finally_correct')
    .eq('user_id', userId)
    .eq('session_id', (
      await supabase
        .from('sessions')
  ...
  const { data: postTest } = await supabase
    ...
    .eq('session_id', (
      await supabase
        ...
        .select('id')
        .eq('user_id', userId)
        .eq('session_type', 'post_test')
        .single()
    )?.data?.id);

  if (!preTest || !postTest) return null;

  const preAccuracy = (preTest.filter((d) => d.user_finally_correct).length / preTest.length) * 100;
  const postAccuracy = (postTest.filter((d) => d.user_finally_correct).length / postTest.length) * 100;
